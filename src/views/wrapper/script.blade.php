<script>
  ...
  document.addEventListener("DOMContentLoaded", function () {
    console.log(
      "\n%cPowered by Nebula - © 2023-2026 Emma (github.com/sdgamer8263-sketch)\n%c\nLearn more at https://skathemes.github.io \nBuyer agreements available at https://cdn.skathemes.github.io/etc/policies/\n\n",

      'font-weight: 900; font-size: 24px; color: #7a98ff;',
      'font-weight: 900; color: #ffffff;'
    )
    @if(Auth::check())
      <?php if($s_website_links == "1") { echo 'insertAboveApp(".ska-weblinks");'; } ?>
      <?php if($s_alert == "1" && $s_website_links != "1") { echo 'insertAboveApp(".alert-spacer");'; } ?>
      <?php if($s_alert == "1") { echo 'insertAboveApp(".ska-alert");'; } ?>
    @endif
  });
</script>
<script>
  ...
  if(params.get('multitasking') == "true") {
    ...
    fetchStyle("/extensions/skatheme/libraries/floatingWindowStyles.css", "Multitasking")
  }
  ...
  async function sidebarRefresh() {
    ...
    if(currentPage.startsWith("account")) {
      ...
      <?php if($s_page_indexing == "1") { echo "if(currentPage === \"accountMore\") {document.getElementById('sidebarAccountMore').className = 'sidebarButton sidebarButtonSelected';} else {document.getElementById('sidebarAccountMore').className = 'sidebarButton';}"; }; ?>
    ...
    if(currentPage.startsWith("server")) {
      ...
      <?php if($s_page_indexing == "1") { echo 'if(currentPage === "serverMore") {document.getElementById(\'sidebarServerMore\').className = \'sidebarButton sidebarButtonSelected\';} else {document.getElementById(\'sidebarServerMore\').className = \'sidebarButton\';}'; } ?>

      setTimeout(() => {
        serverId = fetchServerId()
        sidebarServerButtonRefresh(serverId);
        <?php if($s_page_indexing == "1") { echo "sidebarMoreRefresh()"; } ?>
      }, 300)
    } else {document.getElementById('sidebarCategoryServer').style.display = "none";}

    @if($s_sidebar_always_visible_buttons == "0")
