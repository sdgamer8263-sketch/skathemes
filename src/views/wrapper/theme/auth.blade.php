@if(Auth::check() != true)
<script>
  document.querySelector("head > meta[name='theme-color'][content='#0e4688']").setAttribute("content", "{{ $s_palette_auth_1 }}")
</script>
<!-- PTERODACTYL AUTHENTICATION THEMING -->
<style id="ska-authentication-theme">
  html, body {
  ...
  @keyframes backdrop {
    0%   {scale: calc(.1 +@if($s_auth_background_appearance == "1") 2.4 @else 1 @endif)}
    100% {scale: @if($s_auth_background_appearance == "1") 2 @else 1 @endif}
  }

  @if($s_auth_customlogo != "")
    [class*="LoginFormContainer__Container-sc"] [class*="LoginFormContainer___StyledH-sc"] {
      content: url("{{ $s_auth_customlogo }}");
      border-radius: 10px;
      padding: 0;
      height: 65px;
      max-width: 100%;
  ...
  .ska-auth-wallpaper {
    z-index: 3;
    overflow: hidden;
    @if($s_auth_background_image == "")background-color: var(--authA);
    @else background: url("{{ $s_auth_background_image }}") no-repeat; background-color: #000;@endif
    ...
    top: 0; left: 0;
    position: fixed;
    @if($s_auth_background_appearance == "1")filter: blur(50px);scale: 2;@endif
    @if($s_auth_background_appearance == "1")filter: blur(50px);@endif
    @if($s_auth_background_appearance == "2")opacity: 0.6;@endif
